...
# Author Wei Pan
# 
# STQG solver with ito process for bathymetry
#
...
class TQGSolverRandomBathymetryProcess(object):
    ...
    def __init__(self, tqg_params): 
        ...
        # it is defined as the base bathymetry function h in
        # h dt + zeta \circ W
        ...
        self.psi0 = Function(Vcg, name="Streamfunction")  
        ...
        self.solver_name = 'TQG bathymetry process solver'


    def visualise_h5(self, h5_data_name_prefix,  output_visual_name, time_start=0, time_end=0, time_increment=0, initial_index=0):
        output_file = File(output_visual_name + ".pvd")#, comm=spatial_comm)
        self.load_initial_conditions_from_file(h5_data_name_prefix + "{}".format(initial_index))
        ...
        for t in np.arange(time_increment, time_end, time_increment):
            initial_index += 1
            print(h5_data_name_prefix + "{}".format(initial_index), flush=True)
            self.load_initial_conditions_from_file(h5_data_name_prefix + "{}".format(initial_index))
            v.project(self.gradperp(self.psi0))
            self.ssh.assign(self.psi0 - 0.5 * Function(self.Vcg).project(self.initial_b))
    ...
    def load_initial_conditions_from_file(self, h5_data_name, comm=None):
        ...
        with DumbCheckpoint(h5_data_name, mode=FILE_READ, comm=comm) as chk:
            ...
            try:
                chk.load(self.bathymetry, name="Bathymetry")
            except:
                print('no bathymetry in data')
                #pass
        # PETSc.Sys.Print(norm(self.initial_cond))
    ...
    def solve(self, dumpfreq, output_name, data_output_name, comm_manager, do_save_data=False, do_save_visual=True, do_save_spectrum=False, res=0, zetas_file_name=None, xi_scaling=1, bathymetry_xi=False, **kwargs):
        ...
        #euler_param = euler.EulerParams(Dt, 0.0001, self.Vdg.mesh())
        #euler_solver = euler.EulerSolver(euler_param)
        #euler_solver.solve_for_q_given_psi(self.bathymetry, euler_solver.initial_cond)
        #euler_solver.initial_cond.project(div(grad(self.bathymetry)))
        #euler_solver.psi_solve_given_q(self.initial_cond)
        #self.bathymetry.assign(euler_solver.psi0)
        ...
        if (procno == 0):
            ...
            if do_save_visual:
                ...
                output_file.write(q0, self.psi0, v, b0, self.ssh, self.bathymetry, time=0)
            ...
            if do_save_data:
                ...
                data_chk.store(self.bathymetry)
        ...
        psi0_perturbation = 0 
        bms = None
        if (self.solver_name == 'TQG bathymetry process solver'):
        ...
        while t < (T - Dt / 2):
            ...
            if 1:
                ...
                self.psi_solver.solve()
                #bathymetry_perturbation = 0 if zetas is None else np.sum((zetas.T * bms).T, axis=0) 
                bathymetry_perturbation = np.sum((zetas.T * bms).T, axis=0) 
                #print(psi0_perturbation, flush=True)
                

                ## TODO: check if we only need to perturb once in each step
                ##       this should be the case since we don't solve for h(t,x,y) in the
                ##       intermediate steps
                self.bathymetry.dat.data[:] += bathymetry_perturbation
                self.b_solver.solve()
                self.q_solver.solve()     
            ...
            if tdump == dumpfreq:
                ...
                if (procno == 0):
                    ...
                    if do_save_visual:
                        output_file.write(q0, self.psi0, v, b0, self.ssh, self.bathymetry, time=_t)
