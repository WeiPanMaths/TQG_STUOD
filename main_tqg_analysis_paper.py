#   Copyright 2020
#
#   Torus example for TQG theoretical paper
#   
#
import sys
...
if __name__ == "__main__":
    ...
    workspace = Workspace("/home/wpan1/Data2/PythonProjects/TQG_Theory_Example/res_{}_alpha_{}".format(nx, alpha if alpha != None else 'none'))
    #workspace = Workspace("/home/wpan1/Data2/PythonProjects/TQG_Theory_Example/res_{}_alpha_{}".format(nx, alpha if alpha != None else 'none'))
    
    mesh = TorusMeshHierarchy(nx, nx, 1., 1., 0, period="both", quad=True,comm=comm).get_fine_mesh()
    
    #params = Example2params(T,dt,mesh, bc='n', alpha=alpha)
    params = Example1params(T,dt,mesh, bc='n', alpha=alpha)
    ...
    np.save(workspace.output_name("kinetic_energy", "data"), kinetic_energy_series)
    np.save(workspace.output_name("potential_energy", "data"), potential_energy_series)
    np.save(workspace.output_name("total_energy", "data"), total_energy_series)
    np.save(workspace.output_name("casimir", "data"), casimir_series)
    np.save(workspace.output_name("non_casimir", "data"), non_casimir_series)
