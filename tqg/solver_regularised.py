...
class RTQGSolver(object):
    ...
    def __init__(self, tqg_params):
        ...
        self.psi0 = Function(Vcg)  # psi tilde -- regularised
        self.rpsi0 = Function(Vcg) # psi
        ...
        rpsi_problem = LinearVariationalProblem(Apsi, Lpsi, self.rpsi0, model_params.bcs)
        self.rpsi_solver = LinearVariationalSolver(rpsi_problem, \
            solver_parameters={
                'ksp_type':'cg',
                'pc_type':'sor'
            }
        ...
        Apsi = (alphasqr*dot(grad(phi), grad(psi)) + phi * psi) * dx
        Lpsi = self.rpsi0 * phi * dx 
        ...
        # a_int_ = ( dot(grad(p_), -self.gradperp(self.psi0)*(q - self.db2)))*dx 
    ...
    def solve(self, dumpfreq, output_name, data_output_name, ensemble_comm, do_save_data=False, do_save_visual=True, do_save_spectrum=False, res=0):
        ...
        :return: 
        """
        PETSc.Sys.Print('RTQG solver', flush=True)
        ...
        self.psi0.rename("StreamfunctionRegularised")
        self.rpsi0.rename("Streamfunction")
        b0.rename("Buoyancy")
        ...
        # compute_total_energy = lambda _w, _psi, _f, _h, _b: assemble(  (dot(grad(_psi), grad(_psi)) + _psi*_psi - _h * _b)*dx     )
        compute_total_energy = lambda _rpsi, _psi, _f, _h, _b: assemble(  0.5*(dot(grad(_rpsi), grad(_psi)) + _rpsi*_psi - _h * _b)*dx     )
        #compute_kinetic_energy = lambda _w, _psi, _f: assemble( (dot(grad(_psi), grad(_psi)) + _psi*_psi)*dx )
        #compute_potential_energy = lambda _h, _b : assemble( - _h*_b *dx)
        ...
        if (procno == 0):
            ...
            _te = compute_total_energy(self.rpsi0, self.psi0, self.rotation, self.bathymetry, b0)
        ...
        while t < (T - Dt / 2):
            ...
            if tdump == dumpfreq:
                ...
                if (procno == 0):
                    ...
                    if do_save_spectrum:
                        index += 1
                        np.save(data_output_name + "_energy_{}".format(index), np.asarray(v.at(mesh_grid, tolernace=1e-10)))
                    if do_save_data:
                        data_chk.new_file()
                        data_chk.store(q0)
                        data_chk.store(b0)
