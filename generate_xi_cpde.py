...
cpde_calibration_directory = "CalibrationData-random-bathymetry"
...
class ComputeXi:
    ...
    def compute_eof_data(self):
        ...
        dpsi_firedrake_data_name = workspace.output_name("dpsi_firedrake_data", cpde_calibration_directory)       
    ...
    def compute_eof(self, scale=1): 
        data = np.load(self.workspace.output_name("dpsi_firedrake_data.npy", cpde_calibration_directory))
        print(data.shape)
        cnx = self.cnx
        if 1:
            ...
            self.show_data(lambdas, nums, 'spectrum', self.workspace.output_name('svd_spec_plot',cpde_calibration_directory))
            self.show_data(n_lambs, nums, 'normalised spectrum', self.workspace.output_name('svd_norm_spec_plot',cpde_calibration_directory))
        ...
        output_file = File(self.workspace.output_name("zeta_and_xi_35.pvd", cpde_calibration_directory))
        ...
        output_file = File(self.workspace.output_name("zetas_and_xis.pvd", cpde_calibration_directory))
        output_file.write(zeta0, xi0, zeta1,xi1, zeta2,xi2)


        print(eofs.shape)
        np.save(self.workspace.output_name("zetas",cpde_calibration_directory), eofs)
