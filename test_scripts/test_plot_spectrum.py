...
num_files = 1 # 1 to 100 inclusive 
res = 512
fdataname = lambda n : "/home/wpan1/Data/PythonProjects/TQGExample2/res_512_alpha_none/data/pde_data_{}.npy".format(n)
# fdataname = lambda n : "/home/wpan1/Data/PythonProjects/TQGExample2/res_64_alpha_none/visuals/pde_visuals_energy_{}.npy".format(n)
ibase = 2000 # for 512
...
if False:
    ...
    try:
        ...
        plt.savefig("./test_fine_res_spectrum_time.png")
...
plt.savefig("./test_fine_res_energy_gridvalues_{}.png".format(file_index))
...
print(ps.shape)
im = ax.imshow(np.log(ps), vmin=-4.5, vmax=10)
fig.colorbar(im)
plt.title("energy power spectrum (log scale)")
plt.savefig("./test_fine_res_energy_spectrum_{}.png".format(file_index))
...
print("spatial spectrum animation")
if False:
    plt.style.use('seaborn-pastel')
    fig = plt.figure(4)
    ax = fig.add_subplot(111)
    im = ax.imshow( np.zeros( (int(res/2), int(res/2)) ), vmin=-4.5, vmax=10)
    ...
    def animate(i):
        print("animate", i)
        velocities = np.load(fdataname(10*i+ibase))
        grid_values = 0.5*np.linalg.norm(velocities, ord=2, axis=1)**2 
        grid_values = grid_values.reshape(res+1, res+1).T
        sampled_values = grid_values[:-1, :-1]
        fourier_transform = 2*np.fft.fft2(sampled_values)
    ...
    anim = FuncAnimation(fig, animate, init_func=init, frames=200, interval=200)
    anim.save('./test_fine_res_energy_spectrum.gif', writer='imagemagick')

# spatial spectrum radial animation #####################
print("spatial radial animiation")
if False:
    plt.style.use('seaborn-pastel')
    fig = plt.figure(5)
    ax = fig.add_subplot(111)
    ...
    # plt.savefig("./test_energy_spectrum.jpg")
    anim = FuncAnimation(fig, animate, init_func=init, frames=200, interval=200)
    anim.save('./test_fine_res_energy_spectrum_radial.gif', writer='imagemagick')
