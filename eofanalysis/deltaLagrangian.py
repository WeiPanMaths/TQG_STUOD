...
class DeltaLagragian(TQGFemParams):

    def __init__(self, dt, msh):
        TQGFemParams.__init__(self, dt, msh)

    def compute_delta_streamfunction(self, output_name):
        psi_regularised = None
        psi = Function(self.Vcg, name="Streamfunction")

        # load from file
        for n in range(4001):

        fdataname = lambda n : "/home/wpan1/Data/PythonProjects/TQGExample2/res_512_alpha_none/data/pde_data_{}.npy".format(n)
        dpsi = assemble(lag_f_proj - lag_c_s)
        np.savetxt(output_name, dpsi.dat.data[:], delimiter=",")
