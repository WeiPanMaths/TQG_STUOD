...
cpde_directory = "cPDESolution-random-bathymetry"
fpde_directory = "PDESolution-random-bathymetry"
...
class CoarseGrainedSolutionGenerator(TQGSolver):
    ...
    def __init__(self, tqg_params, comm_manager, workspace, cnx=128, cny=128):
        ...
        self.output_sub_dir = cpde_directory
    ...
    def generate_fine_res_data_at_obs_sites(self, comm_manager, file_index, dumpfreq):
        spatial_comm = comm_manager.comm
        self.load_initial_conditions_from_file(pde_data_fname(self.file_work_space, file_index, fpde_directory), spatial_comm)
...
if __name__ == "__main__":
    ...
    if 0: 
        visual_out = File(wspace.output_name("cpde_data.pvd", cpde_directory+ "/visuals"), comm=spatial_comm)
    ...
    if 0:
        ...
        np.savez(wspace.output_name("obs_data", cpde_directory), **data)
        np.savez(wspace.output_name("obs_data_fs", fpde_directory), **data2)
    ...
    if 0:
        ...
        np.savez(wspace.output_name("obs_data_full_grid", cpde_directory), **data)

    ### generate sub observations at reduced satellite observation sites ####
    if 0:
        # load npz data that was stored in line 324
        all_obs = np.load(wspace.output_name('obs_data.npz', cpde_directory))
        ...
        np.savez(wspace.output_name("obs_data_reduced", cpde_directory), **reduced_data)

    ### generate full res observation data ###
    if 1:
        all_obs_fs = np.load(wspace.output_name('obs_data_fs.npz', fpde_directory))
        sub_obs_site_indices = np.load(wspace.output_name('sub_obs_site_indices.npy', 'ParamFiles'))
        print(sub_obs_site_indices.shape)

        reduced_data_fs = {}
        for _file in all_obs_fs.files:
            print(_file)
            reduced_data_fs[_file] = all_obs_fs[_file][sub_obs_site_indices]
    
        # note that we use cpde_directory below, this is so we minimise changes in the test_particle_filter script
        np.savez(wspace.output_name("obs_data_fs_reduced", cpde_directory), **reduced_data_fs)
