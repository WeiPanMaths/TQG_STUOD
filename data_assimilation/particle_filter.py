...
from stqg.solver_random_bathymetry import STQGSolverRandomBathymetry
...
flag_random_bathymetry = False 
if flag_random_bathymetry:
    STQGSolver = STQGSolverRandomBathymetry
...
class particle_filter_parameters (object):
    def __init__(self, workspace):
        ...
        self.obs_scaling = 200
        #self.obs_period = 1   # multiple of fine resolution dt

        # 0.001 is 35 minutes, or ~0.5 hours, hence
    ...
    def reduced_observation_mesh(self, t, wspace):
        ...
        #sub_obs_site_indices = np.load(wspace.output_name('sub_obs_site_indices.npy', 'ParamFiles')) #wspace.get_parameter_files_dir()))
        sub_obs_site_indices = np.load(wspace.get_parameter_files_dir() + 'sub_obs_site_indices.npy')
...
class particle_filter (object):
    ...
    def __init__(self, particle_filter_params): #, comm = ensemble_comm):
        ...
        self.use_fine_res_obs_flag = False
    ...
    def run_particle_filter_uq_stability(self, _numsteps=5, nproc=25):
        ...
        for _step in range(_numsteps):
            ...
            print("weight_exponents" , weights_exponents)
    ...
    def evolve_particle(self, parent_id, **kwargs):
        ...
        #zeta_fname = self.da_wkspace.output_name("zetas.npy", self.da_wkspace.get_parameter_files_dir())
        #zeta_fname = self.da_wkspace.output_name("zetas.npy", self.da_wkspace.get_zetas_dir())
        zeta_fname = self.da_wkspace.get_zetas_dir() + 'zetas.npy'
        ...
        bm_path = p_i.solve(self.particle_dump_freq, _outputname, _outputname, ensemble_comm, do_save_data=True, do_save_visual=False, do_save_spectrum=True, res=32, bathymetry_xi=True, xi_scaling=0.001)
    ...
    def evolve_particle_nopf(self, parent_id, **kwargs):
        ...
        p_i.solve(self.particle_dump_freq, _outputname, _outputname, ensemble_comm, do_save_data=True, do_save_visual=False, do_save_spectrum=False, res=32, bathymetry_xi=True, xi_scaling=0.001) #zetas_file_name=zeta_fname)
    ...
    def generate_observation(self, t):
        ...
        if self.use_fine_res_obs_flag:
            print(t, "obs_data_fs_reduced", flush=True)
            truth = np.load(self.da_wkspace.get_observation_data_dir() + 'obs_data_fs_reduced.npz')['obs_data_{}'.format(t)]
        else:
    ...
    def get_ensemble_mean(self, uq_flag=False, suffix=''):
        ...
        #return v, pv_mean
        ## self.obs_grid is set in the filtering loop
