...
ensemble= ensemble.Ensemble(COMM_WORLD, 1)
spatial_comm = ensemble.comm
ensemble_comm = ensemble.ensemble_comm


class particle_filter (STQGSolver):
    ...
    def __init__(self, particle_filter_params): #, comm = ensemble_comm):
        ...
        self.obsscaling = self.pf_params.obs_scaling
        self.filter_step = 0
    ...
    def run_particle_filter_one_step(self, comm=ensemble_comm):
        noise_increments = self.solve(....)  # zeta.shape[0] * iter_steps 

        ## get observation
        obs = 0

        ## compute log likelihood
        log_likelihood  self.ssh.at(obs_mesh)

        ## broadcast log likelihood values to proc 0 
    ...
    def run_particle_filter_uq_stability(self, _numsteps=5, nproc=25, twin_experiment_flag=False):
        """
        ...
        #obs_func = self.gen_obs_twin_experiment if twin_experiment_flag else self.gen_obs

        #diagnostics.generate_initial_diagnostics(self, twin_experiment_flag)
        expname = 'twin experiment' if twin_experiment_flag else 'pde experiment'
        ...
        for _step in range(_numsteps):
            ...
            print("\n----- {} d.a. step: {} / {} ".format(expname, self.filter_step+1, _numsteps), " -----------")
        ...
        diagnostics.generate_diagnostics_plots(_num_steps, self.wkspace.get_diagnostic_dir(), self.wkspace.get_test_files_dir())
    ...
    def evolve_particle_nopf(self, parent_id, **kwargs):
        ...
        #p_i.spdesolver(pv_input, _outputname, True, **kwargs)
